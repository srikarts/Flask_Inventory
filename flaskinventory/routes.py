...
def product():
    ...
    if exists== False and request.method == 'GET':
        ...
        try:
            ...
            db.session.rollback()
            flash(f'This product already exits','danger')
            return redirect('/Product')
...
def loc():
    ...
    if exists== False and request.method == 'GET':
    ...
    elif lform.validate_on_submit() and request.method =='POST':
        ...
        try:
            db.session.commit()
            flash(f'Your product location has been updated!','success')
            return redirect(url_for('loc.html'))
        except IntegrityError:
            db.session.rollback()
...
def move():
    ...
    #From here irrespective of if condition
    ...
    if form.validate_on_submit() and request.method =='POST':
        ...
        if boolbeans == False:
            ...
            flash(f'Not enough products in this location. Please add producs.','danger')
        else:
            mov = Movement(ts=timestamp,frm=form.src.data,to = form.destination.data,
                           pname=form.mprodname.data,pqty=form.mprodqty.data)
            db.session.add(mov)
            db.session.commit()
            flash(f'Your activity has been added!','succces')
        return redirect(url_for('move'))
    return render_template('move.html',title='Transfers',form = form, details=details, pdetails=pdetails)
...
def check(frm,to,name,qty):
    if frm==to:
        ...
        if prodq.prod_qty >= qty:
            prodq.prod_qty -= qty
            bal = Balance.query.filter_by(locatin=to,product=name).first()
            a=str(bal)
            if(a=='None'):
        ...
        bl = Balance.query.filter_by(location=frm,product=name).first() # checks if from location is in balance
        a=str(bal)
        if(a=='None'):
            return 'no prod'
            ...
            if a=='None':
                ...
                bl.quantity -= qty
                db.session.commit()
            else: #else add to 'from' qty and minus 'to' qty
